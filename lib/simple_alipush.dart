...
import 'package:simple_alipush/services.dart';

class SimpleAlipush {
  factory SimpleAlipush() {
    if (_instance == null) {
      ...
      _instance = new SimpleAlipush.private(methodChannel, eventChannel);
    }
    return _instance;
  }

  SimpleAlipush.private(this._methodChannel, this._eventChannel);

  final MethodChannel _methodChannel;

  final EventChannel _eventChannel;

  static SimpleAlipush _instance;
  ...
  Future<String> get getDeviceid async {
    final String version = await _methodChannel.invokeMethod('getDeviceid');
    return version;
  }
  ...
  Future<String> unbindAccount() async {
    final String version = await _methodChannel.invokeMethod('unbindAccount');
    return version;
  }
  ...
  Future<String> bindTag(int target, List<String> tags, String alias) async {
    final String version = await _methodChannel.invokeMethod(
        'bindTag', {"target": target, "tags": tags, "alias": alias});
    return version;
  }

  Future<String> unbindTag(int target, List<String> tags, String alias) async {
    final String version = await _methodChannel.invokeMethod(
        'unbindTag', {"target": target, "tags": tags, "alias": alias});
    return version;
  }
  ...
  Future<String> listAliases() async {
    final String version = await _methodChannel.invokeMethod('listAliases');
    return version;
  }
